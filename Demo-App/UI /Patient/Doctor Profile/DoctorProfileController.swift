...
class DoctorProfileController: UIViewController {
    ...
    init(doctor: Doctor, userId : String) {
         self.doctor = doctor
   ...
   let availableSlotsTitle : UILabel = {
    ...
    @objc func buttontapped(_ sender : UIButton){
        let point = CGPoint(x: 0, y: availableSlots.frame.origin.y - scrollView.contentInset.top + 200)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if collectionView.tag == 1 {
           ...
           let cell =  collectionView.cellForItem(at: indexPath)! as! AvailableDates
            ...
            viewController.slotNo = slots.slotno[indexPath.row]
            viewController.starTime = slots.startTime[indexPath.row]
            viewController.endTime = slots.endTime[indexPath.row]
